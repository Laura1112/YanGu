...
//题意：在给定数组中找最大子集，其中任意两个数都可以整除，并返回该子集；
//数组逆序
//nums[j]%nums[i]==0的时候就可以得到一个状态转移方程dp[i] = max(dp[i], dp[j]+1)
//最后通过回溯，把最长子集的元素值加入rst中，并返回
public class Solution {
    public static List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        int curMax = 1,k=0;         // curMax存当前最大子集长度
        int[] par = new int[len];  //可以被当前数整除的最合适的数，即选了这个parent后可以最长  （被除数）
        int[] dp = new int[len];   //可以被当前数整除的数的个数（被除数的个数）
        
        //初始化定义的数组
        for(int i=0;i<len;i++){
        	par[i]=i;	//方便后面回溯判断，直到par[k]=k时表示回溯到自己了，没有父节点了，结束
        	dp[i]=1;	//所有都初始为1，表示当前只可以整除自己
        }
        //动态规划
        for(int i=1;i<len;i++){
        	for(int j=0;j<i;j++){
        		if(ber[j]%ber[i]!=0) continue;
        		if(dp[i]<dp[j]+1){
        			par[i] = j;
        			dp[i] = dp[j]+1;  		
        		}
